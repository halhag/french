...
import { vocabItems, VocabItem } from "../data/vocab";
...
function pickRandom(usedIds: Set<number>): VocabItem | null {
  const available = vocabItems.filter(v => !usedIds.has(v.id));
...
function getScoreMessage(score: number): string {
  const messages = [
    "Zut alors! The words escaped you completely!",
    "1/10... Un mot! C'est un début!",
    "2/10... Deux! You're collecting French words!",
    "3/10... Trois! Keep building that vocabulary!",
    "4/10... Almost halfway! The French dictionary is opening up!",
    "5/10... Moitié-moitié! Half your words are French now!",
    "6/10... Pas mal! Your French word bank is growing!",
    "7/10... Bien! Most words came naturally to you!",
    "8/10... Très bien! Your vocabulary is impressive!",
    "9/10... Presque parfait! One word got away!",
    "10/10... Magnifique! You know all the words!",
  ];
  return messages[score];
}

export function VocabMode({ onBackToMenu }: VocabModeProps) {
  ...
  function loadNextItem(ids: Set<number>) {
    ...
    if (!item) {
      const fresh = vocabItems[Math.floor(Math.random() * vocabItems.length)];
  ...
  function handleSubmit() {
    ...
    const result = checkAnswer(userInput, currentItem.french, currentItem.alternatives ?? []);
